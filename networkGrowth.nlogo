;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Assigment        : Agent-based simulation of Complex Systems                                                                                               ;
; Question number  : 2. Growing network of colored agents where each agent is connected to (some) other agents of a DIFFERENT color.                         ;
; submitted by     : jaipuriyarp                                                                                                                             ;
; NetLogo Version  : 6.2.2                                                                                                                                   ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Parameters Information:                                                                                                                                    ;
; setup                                : Initialization sequence, starts the network run.                                                                    ;
; go-once                              : creates the network once i.e. one agent one connection(iteratively).                                                ;
; go                                   : forever button: if enabled, keeps creating growing network.                                                         ;
; redo-layout                          : forever button: if enabled, changes layout features.                                                                ;
; resize nodes                         : forever button: if enabled, resizes all nodes to 'Agent_Size' parameter.                                            ;
; Agent-Colors                         : provides range from 0 to the parameter value set for Agent-colors, agents are colored within the range.             ;
; layout?                              : if enabled, layout features works else, the agents will be growing in still motion.                                 ;
; Maximum-num-of-connections-per_agent : set maximum number of connections/links for agents.                                                                 ;
; networkGrowth_less-Dense?            : if enabled, it makes network growth less dense if Maximum-num-of-connections-per_agent is set to high value(>5/25). ;
; Agent-Size                           : set Agent size for all agents. It works only if resize_nodes is enabled.                                            ;
; Number_of_Agents                     : set total number of agents in the network.                                                                          ;
; plot?                                : if enabled, shows the degree distribution of nodes/degree and log relationship in the graph.                        ;
; # of agents                          : displays number of agents in the network.                                                                           ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
globals [
 max_num_connections
]
to setup
  ...
  set max_num_connections   Maximum-num-of-connections-per_agent
  if Number_of_Agents > 0 [
    create_new_agent nobody
  ]
  if Number_of_Agents > 1 [
    create_new_agent turtle 0
  ]
  reset-ticks
end

to go
 if count turtles > Number_of_Agents - 1 [
    stop
  ];
 ask links [set color grey]
 create_new_agent find-agent
 tick
  if layout? [
    layout
  ]
...
to create_new_agent [old_agent]
  create-turtles 1 [

    ifelse old_agent = nobody [
      set color one-of [red yellow pink white violet magenta cyan green blue brown grey]
    ]
    [
      let colors_num Agent-Colors
      set color ([color] of old_agent +  random colors_num)
      ;if last [color] of turtles != [color] of old_agent[
      if max_num_connections > 0 [
        create-link-with old_agent [set color gray]
      ]
      ;];
      move-to old_agent
      fd 4
...
to-report find-agent
   ;ask one-of turtles [
     ;show my-links
     ;show count my-links
  ;]
  ; density of network
  let links_limit max_num_connections
  if networkGrowth_less-Dense? [
    ifelse max_num_connections > 25 [
      set links_limit max_num_connections / 5
    ]
    [
      if max_num_connections > 5 [
        set links_limit max_num_connections / 2
      ]
      ; else no use of less-dense
    ]
  ]
  ; finds the turtles with number of connections less than number of connections per agent
  report one-of turtles with [count my-links < links_limit]
end

to resize-nodes
  ask turtles [set size Agent_Size]
end
...
to layout
  ...
  let x_offset max [xcor] of turtles + min [xcor] of turtles
  let y_offset max [ycor] of turtles + min [ycor] of turtles

  set x_offset box_size x_offset 0.1
  set y_offset box_size y_offset 0.1
  ; offset calculation taken from models-library
  ask turtles [ setxy (xcor - x_offset / 2) (ycor - y_offset / 2) ]
end

to-report box_size [num bound]
  if num > bound [
    report bound
  ]
  if num < (- bound) [
...
; plot? this code section is picked from models-library
@#$#@#$#@
GRAPHICS-WINDOW
345
10
...
260
258
Maximum-num-of-connections-per_agent
20.0
1
0
Number
